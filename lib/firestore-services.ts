...
// Update an event
export async function updateEvent(eventId: string, updates: Partial<PastEvent>) {
  try {
    const eventRef = doc(db, EVENTS_COLLECTION, eventId)
...
export async function addMediaToEvent(eventId: string, mediaItems: GalleryItem[]) {
  try {
...
export async function removeMediaFromEvent(eventId: string, mediaIndex: number) {
  try {
    const event = await getEventById(eventId)
    if (!event) {
      return { success: false, error: "Event not found" }
